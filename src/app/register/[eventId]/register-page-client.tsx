...
type RegisterApiResponse = {
  status?: "signup_required" | "already_registered" | "success";
  member?: Member;
  event?: Event;
  error?: string;
};
...
export default function RegisterPageClient() {
  ...
  async function fetchEvent() {
    ...
    const response = await fetch(
      `/api/register?eventId=${encodeURIComponent(eventId)}`,
      {
        method: "GET",
        cache: "no-store",
      },
  ...
  async function handleNetIdSubmit(e: React.FormEvent) {
    ...
    if (!response.ok) {
      setErrorMsg(payload.error ?? "Could not continue registration.");
      return;
    }
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (payload.status === "success" && payload.member) {
  ...
  return (
    <div
      className={`min-h-[100dvh] w-full flex flex-col items-center justify-center bg-gradient-to-b from-[#00031A] to-[#001F5B] px-4 py-8 ${changa.className}`}
    >
      <img
        src="/images/shpe-logos/shpe-emblem-transparent.png"
        ...
        alt="SHPE Logo"
      />
      <h1 className="text-2xl sm:text-4xl font-bold text-white mb-1 text-center">
        Register for {event?.name}
      </h1>
      <p className="text-[#A4C2FF] mb-4 text-center text-sm">
        {event ? new Date(event.date).toLocaleDateString() : ""} {event ? `· ${event.points_value} pts` : ""}
      </p>
      ...
      {step === "netid" && (
        <form onSubmit={handleNetIdSubmit} className="flex flex-col items-center gap-3 w-full max-w-md">
          ...
          <button type="submit" className="w-full sm:w-auto bg-[#FD652F] hover:bg-[#e65516] text-white font-semibold px-8 py-2 rounded-full shadow-md transition">
            Continue
          </button>
        </form>
      )}
      ...
      {step === "already" && (
        <div className="text-center max-w-md">
          <p className="text-3xl text-[#FD652F] font-bold">Already registered</p>
          <p className="text-[#A4C2FF] mt-2">You&apos;re already registered with SHPE Cornell!</p>
          <button type="button" onClick={resetToNetIdEntry} className="mt-4 text-[#A4C2FF] text-sm underline">
            ← Back to Net ID
          </button>
        </div>
      )}
      ...
      {step === "signup" && (
        <form onSubmit={handleSignup} className="flex flex-col gap-3 w-full max-w-2xl max-h-[calc(100dvh-11.5rem)] overflow-y-auto pr-1 pb-2">
          <div className="text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input type="text" placeholder="First Name" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="w-full px-3 py-2 text-sm text-white bg-[#002F6C] border-2 border-[#FD652F] rounded-lg outline-none placeholder:text-[#A4C2FF]" />
            <input type="text" placeholder="Last Name" value={lastName} onChange={(e) => setLastName(e.target.value)} required className="w-full px-3 py-2 text-sm text-white bg-[#002F6C] border-2 border-[#FD652F] rounded-lg outline-none placeholder:text-[#A4C2FF]" />
          </div>
            ...
            <div className="relative">
              <div className="max-h-[10rem] overflow-y-auto pr-1 pb-12 space-y-1.5">
                {orderedMajorOptions.map((majorOption) => {
                  ...
                  return (
                    <div key={majorOption.value} className={`w-full rounded-2xl border transition-colors ${isSelected ? "bg-[#FD652F] text-white border-[#FD652F]" : "bg-[#001A40] text-[#E5EFFF] border-[#0070C0]"}`}>
                      {isOther && isSelected ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            />
            <select value={gradSemester} onChange={(e) => setGradSemester(e.target.value)} className="w-full px-3 py-2 text-sm text-white bg-[#002F6C] border-2 border-[#FD652F] rounded-lg outline-none">
          ...
          <button type="submit" aria-disabled={submitting} className="w-full bg-[#FD652F] hover:bg-[#e65516] active:bg-[#d94f14] active:brightness-95 active:shadow-[inset_0_2px_6px_rgba(0,0,0,0.28)] text-white text-sm font-semibold px-8 py-2 rounded-full shadow-md transition-colors duration-75">
      ...
      {step === "success" && (
        <div className="flex flex-col items-center gap-2 w-full max-w-xl">
          ...
          <p className="text-3xl sm:text-4xl font-bold text-white text-center">Welcome, {member?.first_name}!</p>
          <p className="text-[#A4C2FF] text-base sm:text-lg text-center mb-1">
            Registration complete — <span className="text-[#FD652F] font-bold">+{event?.points_value} pts</span> awarded!
          </p>
          <p className="text-[#85B6FF] text-sm sm:text-base text-center mt-1">Ready for the next registration...</p>
        </div>
      )}
    </div>
  );
