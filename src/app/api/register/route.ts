...
export async function POST(request: NextRequest) {
  ...
  const insertPayload: {
    ...
    last_name: lastName,
    email: `${normalizedNetId}@cornell.edu`,
    major,
  ...
  try {
    await markAttendance(createdMember.id, eventId);
  } catch (error: unknown) {
    const code = (error as { code?: string })?.code;

    if (code !== "23505") {
      return NextResponse.json(
        { error: "Could not complete registration check-in. Please try again." },
        { status: 500, headers: guard.headers },
      );
    }
  }

  return NextResponse.json({ status: "success", member: createdMember }, { headers: guard.headers });
}
